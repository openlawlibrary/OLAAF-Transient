...
def initialize_hashes(repo_path, initial_commit=None):
  ...
  # TODO
  # If edition corresponds to one commit
  # Do we even need it as an entity?
  # If we want to have both entities, then create a new edition for every commit here?
  # Commit and edition should then be in a one-to-one relationship
  # Keeping it like this while developing the transient authentication
